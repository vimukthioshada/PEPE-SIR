...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Mr™ Bot Ofc.`, "footerText": "*_𝙿𝙴𝙿𝙴 𝚂𝙸𝚁_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*Mr™*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             xeon.relayWAMessage(res)
        }
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Mr™`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/94703148103?text=hello`}}})
		}
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/94703148103`,
                "mediaType": "10",
                "mediaUrl": `wa.me/94703148103`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/94703148103`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "94703148103@s.whatsapp.net";
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "94703148103@s.whatsapp.net"
        dtod = '94703148103'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
│
│  ❑ *ɢɪᴛʜᴜʙ ʟɪɴᴋ* :
│  https://github.com/Mr™sir/Mr™-SIR
│  
│  ❑ *ʏᴏᴜᴛᴜʙᴇ ʟɪɴᴋ* :
...
Mr™x = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideosearch?query=${c}&apikey=${HunterApi}`)
reply(mess.wait)
Mr™x = Mr™x.result
ini_txt = ""
for (var x of Mr™x) {
...
case 'waifu':
  ...
  break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Mr™`)
